...
export default function ManageUsersScreen() {
  ...
  const handleAddUser = async () => {
    ...
    if (formData.permission_level === 'department' && formData.department_ids.length === 0) {
      if (Platform.OS === 'web') {
        ...
        Alert.alert('خطأ', 'يرجى اختيار قسم واحد على الأقل');
      }
      return;
    }
  ...
  // السماح للمدير فقط أو من لديه صلاحية إدارة المستخدمين
            ...
            <View style={styles.userActions}>
              ...
              {user.role !== 'admin' && (
                <TouchableOpacity
                  ...
                  accessibilityLabel={user.is_active ? "إيقاف الحساب" : "تفعيل الحساب"}
                >
              ...
              <TouchableOpacity
                ...
                accessibilityLabel="إعادة تعيين كلمة المرور"
              >
                  ...
                  <>
                    ...
                    {formData.permission_level === 'department' && formData.faculty_id && formData.faculty_ids.length <= 1 && (
                      <>
                        ...
                        <Text style={styles.hintText}>
                          اضغط على القسم لتحديده أو إلغاء تحديده
                        </Text>
                        <View style={styles.scopeSelector}>
                          {departments
                            ...
                            .map(dept => {
                              ...
                              return (
                                <TouchableOpacity
                                  ...
                                  style={[
                                    styles.scopeItem,
                                    isSelected && styles.scopeItemActive
                                  ]}
                                  onPress={() => {
                                    setFormData(prev => {
                                      const currentIds = prev.department_ids || [];
                                      if (currentIds.includes(dept.id)) {
                                        return { ...prev, department_ids: currentIds.filter(id => id !== dept.id) };
                                      } else {
                                        return { ...prev, department_ids: [...currentIds, dept.id] };
                                      }
                                    });
                                  }}
                                >
                                  <View style={styles.checkboxContainer}>
                                    ...
                                    <Text style={[
                                      styles.scopeItemText,
                                      isSelected && styles.scopeItemTextActive
                                    ]}>
                                      {dept.name}
                                    </Text>
                                  </View>
                        ...
                        {formData.department_ids.length === 0 && departments.filter(d => d.faculty_id === formData.faculty_id).length > 0 && (
                          <Text style={styles.requiredHint}>⚠️ يجب اختيار قسم واحد على الأقل</Text>
                        )}
                        {formData.department_ids.length > 0 && (
                          <Text style={styles.selectedCountText}>
                            تم اختيار {formData.department_ids.length} قسم
                          </Text>
                        )}
                      </>
                    )}
