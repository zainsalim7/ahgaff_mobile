...
export default function OfflineSyncScreen() {
  ...
  const formatDateStr = (timestamp: string) => {
    const date = new Date(timestamp);
    const dateStr = formatGregorianDate(date, { includeYear: true });
    ...
    return (
      <View style={[styles.recordCard, item.synced && styles.recordCardSynced]}>
        ...
        <View style={styles.recordFooter}>
          <Text style={styles.recordTime}>{formatDateStr(item.timestamp)}</Text>
          {item.sync_error && (
          ...
          <View style={styles.connectionInfo}>
            ...
            {lastSyncTime && (
              <Text style={styles.lastSync}>
                آخر مزامنة: {formatDateStr(lastSyncTime)}
              </Text>
            )}
          </View>
          {!isOnline && (
