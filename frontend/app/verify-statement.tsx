...
// صفحة تحقق عامة من صحة إفادة الطالب — بدون تسجيل دخول
export default function VerifyStatementPage() {
  ...
  useEffect(() => {
    const run = async () => {
      try {
        ...
        const res = await axios.get(`${base}/api/verify/statement/${token}`);
        setResult(res.data);
      } catch {
  ...
  return (
    <View style={{ flex: 1, backgroundColor: '#f4f6fa', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
      <View style={{ backgroundColor: '#fff', borderRadius: 16, padding: 28, width: '100%', maxWidth: 460, alignItems: 'center', shadowColor: '#000', shadowOpacity: 0.08, shadowRadius: 16 }} testID="verify-statement-card">
        <Text style={{ fontSize: 18, fontWeight: '800', color: '#1a2540', marginBottom: 4 }}>جامعة الأحقاف</Text>
        <Text style={{ fontSize: 12, color: '#8a95a8', marginBottom: 18 }}>التحقق من صحة إفادة طالب</Text>
        {loading ? (
          ...
          <>
            ...
            <Text style={{ fontSize: 15, fontWeight: '800', color: '#c62828', marginTop: 10, textAlign: 'center' }} testID="verify-invalid">
              {result?.message || 'لا توجد إفادة بهذا الرمز'}
            </Text>
          </>
        )}
      </View>
