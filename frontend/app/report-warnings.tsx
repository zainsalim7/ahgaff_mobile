...
import { exportToPDF, prepareWarningsData } from '../src/utils/pdfExport';
...
export default function WarningsReport() {
  ...
  const handleExportPDF = async () => {
    try {
      ...
      const allStudents = [...warnings.map(s => ({...s, status: 'warning'})), ...deprivations.map(s => ({...s, status: 'deprived'}))];
      const reportData = prepareWarningsData(allStudents);
          ...
          {Platform.OS === 'web' && (
            ...
            >
              {exportingPDF ? (
                ...
                <Ionicons name="document-text-outline" size={22} color={(warnings.length > 0 || deprivations.length > 0) ? "#e53935" : "#ccc"} />
              )}
            </TouchableOpacity>
          )}
          <TouchableOpacity 
